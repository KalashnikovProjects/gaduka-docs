...
- 'размер' - необязательный аргумент, размер текста, по умолчанию 50

- 'цвет' - необязательный аргумент, цвет текста, по умолчанию чёрный

.. code-block:: python 

    добавить_текст(изображение, (0.5, 0.5), "Привет Мир!", размер=30, цвет=зелёный)
...
.. code-block:: python 
    ...
    добавить_линию(изображение, (левый_верхний_угол, (0.3, 0.3)), ширина=5)

добавить_многоугольник
~~~~~~~~~~~~~~
...
- 'углы' - координаты углов многоугольника, в списка координат
...
.. code-block:: python 
    ...
    добавить_многоугольник(изображение, (левый_верхний_угол, (0.3, 0.3), правый_верхний_угол), цвет=красный, обводка=зелёный)

добавить_прямоугольник
~~~~~~~~~~~~~~
...
.. code-block:: python 
    ...
    добавить_прямоугольник(изображение, (0.5, 0.5), ширина=0.3, высота=0.4, обводка=зелёный, ширина_обводки=10)

добавить_круг
~~~~~~~~~~~~~~

Накладывает круг на изображение.
...
.. code-block:: python 
    ...
    добавить_круг(изображение, (0.5, 0.5), 0.3, обводка=зелёный, ширина_обводки=10)
...
Принимает аргументы:
- 'изображение' - какое изображение обработать

- 'углы' - координат границ обрезки в пикселях (левая, верхняя, правая, нижняя)

.. code-block:: python 

    изображение = обрезать(изображение, (10, 590, 10, 590))
...
- 'коэффициент' - на сколько сжать (от 1 до 15, рекомендуемое - 2)

.. code-block:: python 

    изображение = сжать(изображение, 2)
...
- 'поворот' - на сколько градусов повернуть (против часовой стрелки)

.. code-block:: python 

    изображение = повернуть(изображение, 90)
...
- 'по_вертикали' - Верно/Неверно

- 'по_горизонтали' - Верно/Неверно

.. code-block:: python 

    изображение = отразить(изображение, по_горизонтали=Верно, по_вертикали=Верно)

эффекты
~~~~~~~~

Накладывает эффекты на изображение

Принимает аргументы:
- 'изображение' - какое изображение обработать

- 'эффекты' - список эффектов, может содержать следующие переменные: [блюр, выпуклость, серость, границы]

.. code-block:: python 

    изображение = эффекты(изображение, [блюр, выпуклость, серость, границы])
